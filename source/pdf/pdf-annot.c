...
pdf_load_link(fz_context *ctx, pdf_document *doc, pdf_obj *dict, const fz_matrix *page_ctm)
{
	...
	else
	{
		...
		{
			action = pdf_dict_gets(ctx, dict, "RichMediaContent");
			if(action)
			{
				assetobj = pdf_dict_gets(ctx, action, "Assets");
				if(assetobj)
				{
					namesobj = pdf_dict_gets(ctx, assetobj, "Names");
					if(namesobj)
					{
						arraycount = pdf_array_len(ctx, namesobj);
						for(i=0; i<arraycount; i++)
						{
							onetagobj = pdf_array_get(ctx, namesobj, i);
							if(onetagobj)
							{
								efobjtag = pdf_dict_gets(ctx, onetagobj, "EF");
								if(efobjtag)
								{
									ftagobj = pdf_dict_gets(ctx, efobjtag, "F");
									if(ftagobj)
									{
										if(pdf_is_stream(ctx, ftagobj))
										{
											filename[loopcount] = pdf_to_utf8(ctx, doc, pdf_dict_gets(ctx, onetagobj, "F"));
											strmbuf[loopcount] = pdf_load_stream(ctx, doc, pdf_to_num(ctx, ftagobj), pdf_to_gen(ctx, ftagobj));
						...
						if(arraycount == 6)
						{
							if(strmbuf[0]->len>strmbuf[1]->len && strmbuf[0]->len>strmbuf[2]->len)
							{
								...
								fz_free(ctx, filename[1]);
								fz_free(ctx, filename[2]);
								fz_drop_buffer(ctx, strmbuf[1]);
								fz_drop_buffer(ctx, strmbuf[2]);
							}
							else if(strmbuf[1]->len>strmbuf[2]->len)
							{
								...
								fz_free(ctx, filename[0]);
								fz_free(ctx, filename[2]);
								fz_drop_buffer(ctx, strmbuf[0]);
								fz_drop_buffer(ctx, strmbuf[2]);
							}
							else
							{
