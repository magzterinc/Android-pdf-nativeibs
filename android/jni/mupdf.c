...
//	fz_link *link;
	...
	linknode = fz_load_links(glo->doc, pc->page);

	for (; linknode; linknode = linknode->next)
	{
		if (p.x >= linknode->rect.x0 && p.x <= linknode->rect.x1)
			if (p.y >= linknode->rect.y0 && p.y <= linknode->rect.y1)
				break;
	}
	...
	if (linknode->dest.kind == FZ_LINK_URI)
		return (*env)->NewStringUTF(env, linknode->dest.ld.uri.uri);
	else if(linknode->dest.kind == FZ_LINK_MEDIA)
		return (*env)->NewStringUTF(env, "MEDIA");

	return NULL;
}


//This is for reading richmedia content and this will write the embedded audio/video file to the writepath parameter
JNIEXPORT jstring JNICALL
Java_com_artifex_mupdf_MuPDFCore_getMediaStream(JNIEnv * env, jobject thiz, jstring writepath)
{
	int i=0;
	char *folderpath;
	char *savepath=NULL;

	folderpath = (*env)->GetStringUTFChars(env, writepath, NULL);
	savepath = malloc(strlen(folderpath)+strlen(linknode->dest.ld.media.filename)+1);
	strcpy(savepath,folderpath);
	strcat(savepath,linknode->dest.ld.media.filename);

	FILE* fileptr;
	if((fileptr=fopen(savepath,"r"))==NULL)
	{
		fileptr = fopen(savepath,"w");
		fwrite(linknode->dest.ld.media.mediastream->data , 1, linknode->dest.ld.media.mediastream->len, fileptr);
		fclose(fileptr);
	}
	else
	{
	...
	return (*env)->NewStringUTF(env, savepath);
}
